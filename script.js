...
const serbatoi = {
  ...
  function getDensita(serbatoio, temperatura) {
    const densitaList = serbatoi[serbatoio].densita;
    const tempIndex = Math.min(Math.max(0, Math.round(temperatura)), 34);
    return densitaList[tempIndex];
  }
  ...
  const converti = () => {
    ...
    if (tipoConversione === 'toTonnellate') {
      ...
      risultatoDiv.textContent = `${valore} mm equivalgono a ${risultato.toLocaleString()} Kg.`;
    } else if (tipoConversione === 'toMillimetri') {
      // Converti tonnellate in mm
      risultato = Math.round(valore / (K * densita));
      risultatoDiv.textContent = `${valore} Kg equivalgono a ${risultato.toLocaleString()} mm.`;
    }
  };
